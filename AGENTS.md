...
- `likes`（用户对咖啡豆的点赞记录）
  ...
  -- 唯一约束：同一用户对同一咖啡豆只能点赞一次
  create unique index likes_user_id_bean_id_key on likes (user_id, bean_id);
  -- 性能索引
  create index likes_user_id_idx on likes (user_id);
  create index likes_bean_id_idx on likes (bean_id);
  create index likes_created_at_idx on likes (created_at desc);
  ```
  - 已启用 RLS，策略（仅 `authenticated` 角色生效）：
    - `SELECT`：允许（任意登录用户可读取全部点赞，用于热门排行）
    - `INSERT`：`with check (auth.uid() = user_id)`，只能插自己的点赞
    - `DELETE`：`using (auth.uid() = user_id)`，只能取消自己的点赞
    - `UPDATE`：无策略，默认拒绝（点赞记录不可修改）
...
- 点赞相关 API（在 `app/api/likes/...` 下创建）：
  - 推荐直接使用 Supabase 浏览器端客户端（`@supabase/ssr`）调用 `likes` 表，受 RLS 保护，无需额外 Route Handler。
  - 常见操作：
    - 点赞：`insert({ user_id: session.user.id, bean_id })` —— 唯一约束保证幂等
    - 取消点赞：`delete().eq('user_id', session.user.id).eq('bean_id', bean_id)`
    - 某豆的点赞数：`select('*', { count: 'exact', head: true }).eq('bean_id', beanId)`
    - 当前用户是否点过：`select('id').eq('user_id', session.user.id).eq('bean_id', beanId).maybeSingle()`
  - 若必须用 service role 走服务端 API，禁止把 user_id 写死或从前端请求体中直接读取再插入——必须从 session 中取 `auth.uid()` 后再写入。
